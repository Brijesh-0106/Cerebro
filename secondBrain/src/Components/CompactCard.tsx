...
export const CompactCard = ({
  ...
  return (
    <>
      <span className="mb-6 break-inside-avoid flex max-w-60 flex-col rounded-xl border-2 gap-1 py-2 border-zinc-200 dark:border-[#a9a9a9] bg-white dark:bg-transparent shadow-sm dark:shadow-none px-4">
        {/* Header */}
        <div className="flex justify-between items-center">
          <span className="text-zinc-500 dark:text-[#a9a9a9]">
            {type === "youtube" ? (
              ...
              <GiNotebook size={16} className="text-primary dark:text-[#E6D8F2]" />
            )}
          </span>
          <span className="text-zinc-500 dark:text-[#a9a9a9] text-xs">{createdAt}</span>
        </div>
        ...
        {/* Description */}
        <div className="text-zinc-600 dark:text-[#a9a9a9] text-sm">
          {description.length > 42
            ? description.slice(0, 42) + "..."
            : description}
        </div>
        <div className="flex justify-between">
          <span className="text-zinc-900 dark:text-white text-sm">Source {courceNo}</span>
          <NavLink
